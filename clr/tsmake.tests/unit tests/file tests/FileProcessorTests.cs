...
that spans down to here */ SELECT 'line 4' [4th line]; -- also a comment";
    ...
    #region Tokens
    public const string SIMPLE_TOKEN_BUILD_FILE_WITH_INCLUDE = @"-- 1
...
DECLARE @currentVersion sysname = N'{{##VERSION}}';
    ...
    public const string INCLUDE_FILE_WITH_TOKENS = @"-- 25

-- {{##PROJECT_LINK:https://www.totalsql.com/tools/xxx}}
 ";
    #endregion
...
-- 17";
    ...
    public const string BUILD_FILE_WITH_FILE_INCLUDE = @"-- ## ROOT: ..\ ##:: 
-- ## OUTPUT: \\\my_project.sql
...
DECLARE @currentVersion sysname = N'{{##VERSION}}';
 ...
 --##FILE: include1.sql ";
    ...
    public const string INCLUDE_FILE_WITH_SUB_INCLUDE = @"/* Sample 
header comment
here */ 
CREATE PROC dbo.MyProc AS SELECT @@VERSION;
GO 
...
-- ## FILE: include2.sql       ";
    ...
    public void ProcessLines_Bubbles_Simple_Token_Up_To_Caller()
    {
        ...
        Assert.That(token1.Location.Peek().LineNumber, Is.EqualTo(3));
        Assert.That(token1.Location.Peek().Column, Is.EqualTo(36));
        StringAssert.AreEqualIgnoringCase(@"VERSION", token1.Name);
        Assert.Null(token1.DefaultValue);
    }

    // TODO: 
    // ProcessLines_Bubles_Complex_(i.e., more than 1)_Tokens... 

    [Test]
    public void ProcessLines_Captures_Single_CodeFile_For_Single_File_Build()
    {
        ...
        var result = FileProcessor.ProcessFileLines(null, "basic_build_file.sql", ProcessingType.BuildFile, fileManager.Object, "NA", "");

        Assert.That(result.CodeFiles.Count, Is.EqualTo(1));
        StringAssert.AreEqualIgnoringCase(@"basic_build_file.sql", result.CodeFiles[0].FileName);
    }

    [Test]
    public void ProcessLines_Captures_Multiple_CodeFiles_For_Multi_File_Build()
    {
        var fileBody = BUILD_FILE_WITH_FILE_INCLUDE;
        var fileLines = Regex.Split(fileBody, @"\r\n|\r|\n", Global.StandardRegexOptions).ToList();

        var include1Body = INCLUDE_FILE_WITH_SUB_INCLUDE;
        var include1Lines = Regex.Split(include1Body, @"\r\n|\r|\n", Global.StandardRegexOptions).ToList();

        var include2Body = ULTRA_SIMPLE_INCLUDE_FILE; 
        var include2Lines = Regex.Split(include2Body, @"\r\n|\r|\n", Global.StandardRegexOptions).ToList();

        var fileManager = new Mock<IFileManager>();
        fileManager.Setup(x => x.GetFileContent("my_build.sql"))
            .Returns(fileBody);

        fileManager.Setup(x => x.GetFileLines("my_build.sql"))
            .Returns(fileLines);

        fileManager.Setup(x => x.TranslatePath("include1.sql", PathType.Relative, It.IsAny<string>(), It.IsAny<string>()))
            .Returns("include1.sql");

        fileManager.Setup(x => x.FileExists("include1.sql"))
            .Returns(true);

        // TODO: if I omit the following line, I get a stack-overflow exception (i.e., what's my code doing IF it can't find this file?)
        fileManager.Setup(x => x.GetFileContent("include1.sql"))
            .Returns(include1Body);

        fileManager.Setup(x => x.GetFileLines("include1.sql"))
            .Returns(include1Lines);

        fileManager.Setup(x => x.TranslatePath("include2.sql", PathType.Relative, It.IsAny<string>(), It.IsAny<string>()))
            .Returns("include2.sql");

        fileManager.Setup(x => x.FileExists("include2.sql"))
            .Returns(true);

        fileManager.Setup(x => x.GetFileContent("include2.sql"))
            .Returns(include2Body);

        fileManager.Setup(x => x.GetFileLines("include2.sql"))
            .Returns(include2Lines);

        // TODO:
        // WARN:
        //      I'm passing this in as a .IncludedFIle (argument) vs a .BuildFile. That's NOT really how this was designed.
        var result = FileProcessor.ProcessFileLines(null, "my_build.sql", ProcessingType.IncludedFile, fileManager.Object, "NA", "");

        Assert.That(result, Is.Not.Null);
        Assert.That(result.Errors.Count, Is.EqualTo(0));
        Assert.That(result.Tokens.Count, Is.EqualTo(1));
        
        //Assert.That(result.Lines.Count, Is.EqualTo(16));
        Assert.That(result.CodeFiles.Count, Is.EqualTo(3));

    }

    [Test]
    public void ProcessLines_Captures_Tokens_And_Locations_From_Nested_Files()
    {
        var fileBody = SIMPLE_TOKEN_BUILD_FILE_WITH_INCLUDE;
        var fileLines = Regex.Split(fileBody, @"\r\n|\r|\n", Global.StandardRegexOptions).ToList();

        var includeBody = INCLUDE_FILE_WITH_TOKENS;
        var includeLines = Regex.Split(includeBody, @"\r\n|\r|\n", Global.StandardRegexOptions).ToList();

        var fileManager = new Mock<IFileManager>();
        fileManager.Setup(x => x.GetFileContent("build.sql"))
            .Returns(fileBody);

        fileManager.Setup(x => x.GetFileLines("build.sql"))
            .Returns(fileLines);

        fileManager.Setup(x => x.TranslatePath(It.IsAny<string>(), PathType.Relative, It.IsAny<string>(), It.IsAny<string>()))
            .Returns("include.sql");

        fileManager.Setup(x => x.FileExists("include.sql"))
            .Returns(true);

        fileManager.Setup(x => x.GetFileContent("include.sql"))
            .Returns(includeBody);

        fileManager.Setup(x => x.GetFileLines("include.sql"))
            .Returns(includeLines);

        // TODO:
        // WARN:
        //      I'm passing this in as a .IncludedFIle (argument) vs a .BuildFile. That's NOT really how this was designed.
        var result = FileProcessor.ProcessFileLines(null, "build.sql", ProcessingType.IncludedFile, fileManager.Object, "NA", "");

        Assert.That(result, Is.Not.Null);
        Assert.That(result.Errors.Count, Is.EqualTo(0));
        Assert.That(result.Lines.Count, Is.EqualTo(11));
        Assert.That(result.Tokens.Count, Is.EqualTo(2));

        var token1 = result.Tokens[0];
        Assert.That(token1.Location.Count, Is.EqualTo(1));
        var location1 = token1.Location.Peek();
        Assert.That(location1.LineNumber, Is.EqualTo(6));
        Assert.That(location1.Column, Is.EqualTo(36));
        StringAssert.AreEqualIgnoringCase(@"build.sql", location1.FileName);

        var token2 = result.Tokens[1];
        Assert.That(token2.Location.Count, Is.EqualTo(2));
        location1 = token2.Location.Pop();
        Assert.That(location1.LineNumber, Is.EqualTo(3));
        Assert.That(location1.Column, Is.EqualTo(3));
        StringAssert.AreEqualIgnoringCase(@"include.sql", location1.FileName);

        Assert.That(token2.Location.Count, Is.EqualTo(1));
        var location2 = token2.Location.Peek();
        Assert.That(location2.LineNumber, Is.EqualTo(8));       // NOTE: Line 8 is the FILE directive (i.e., the 1st file in the 'chain' to this token).

        // TODO: for now, I don't care about the position (column-number) of Directives if/when they're referenced in part of a stack/chain of files - i.e., position (column) 0 is GOOD enough. 
        Assert.That(location2.Column, Is.EqualTo(0));
        StringAssert.AreEqualIgnoringCase(@"build.sql", location2.FileName);
    }
